            ...
            <div class="col-lg-6">
              <div class="about-avatar">
                ...
                <img v-if="currentUser.cover!=null"
                     v-bind:src="
              `http://localhost:8100/` + currentUser.cover
            "
                     width="250px"
                     height="250px"
                />
...
export default {
  ...
  methods: {
    ...
    handleTakePhoto() {
      ...
      // }
      // this.sendImage();
      this.faceTime(params)
    },
    faceTime(data) {
      // Axios
      FaceIdentified.faceTimeKeep(data).then(response => {
        if (response.data.local_message === "Chấm công thành công") {
          this.faceSuccess = true;
          // console.log("Res"+response)
          // console.log(this.faceSuccess)
          this.userInfo = response.data.data.person[0];
          this.getUser();
          this.sendImage();
        } else {
    ...
    sendImage() {
      ...
      let code = this.currentUser.code;
      let fullname = this.currentUser.fullName;
    ...
    getClientIp() {
      IpService.getClientIp().then(res => {
        this.clientIp = res.data.ip
        this.checkAuth = res.data.valid
        console.log(this.clientIp + "valid"+this.checkAuth)
      })
