<template>
  <div class="container ">
    <router-link to="/managerequest" class="btn-back"><i class="el-icon-back"></i>&nbsp;&nbsp;Danh sách đề xuất
    </router-link>
    <br><br>
    ...
    <div style="width: 100%">
      ...
      <div v-for="(item, index) in request.approvers" :key="index">
        <button class="btn-accept" v-if="request.approveStatus.id==1 && item.id==currentUser.user.id"
                @click="changeStatus(request.id, 2,request.approveStatus.id, currentUser.user.id)">Chấp thuận
        </button>
        <button class="btn-refuse" v-if="request.approveStatus.id==1 && item.id==currentUser.user.id"
                @click="changeStatus(request.id, 3,request.approveStatus.id,currentUser.user.id)">Từ chối
        </button>
        <button class="btn-undo"
                v-if="(request.approveStatus.id==2 || request.approveStatus.id==3) && item.id==currentUser.user.id"
                @click="changeStatus(request.id, 1,request.approveStatus.id,currentUser.user.id)">Hoàn tác
        </button>
      ...
      <div class="col-6 right">
        ...
        <table>
          ...
          <tr v-if="request.categoryReason.id!=6">
            ...
            <td><span v-if="request.timeStart!=null">{{ request.timeStart }},</span> <span
                v-if="request.dateFrom!=null">Ngày: {{ request.dateFrom }}</span></td>
          </tr>
          <tr v-if="request.categoryReason.id!=6">
            <td class="header-table">Thời gian thực hiện:</td>
            <td><span v-if="request.timeEnd!=null">{{ request.timeEnd }},</span> <span v-if="request.dateTo!=null">Ngày: {{
                request.dateTo
              }}</span>
            </td>
          </tr>
      ...
      <div class="col-6 left">
        ...
        <div v-for="(item, index) in request.approvers" :item="item"
          ...
          <b-avatar style="margin-left: 12px" variant="info" v-if="item.cover != null"
                    v-bind:src="`http://localhost:8100/` + item.cover"></b-avatar>
          <b-avatar style="margin-left: 12px" variant="info" v-if="item.cover == null"
