...
class ExcelService {
    exportExcelReport(params) {
        axios.get(`http://localhost:8080/api/excel/export_report`, {
            params,
            responseType: 'blob',
        }).then((response) => {
    ...
    exportExcelPhep(params) {
        axios.get(`http://localhost:8080/api/excel/export_phep`, {
            params,
            responseType: 'blob',
        }).then((response) => {
            const url = URL.createObjectURL(new Blob([response.data]))
    ...
    exportExcelUser(params) {
        axios.get(`http://localhost:8080/api/excel/export_users`, {
            params,
            responseType: 'blob',
        }).then((response) => {
            const url = URL.createObjectURL(new Blob([response.data]))
